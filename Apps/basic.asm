...
read_args:
	inc 	word[index]
	mov 	cx, [index]				; argv index
	mov 	dx, my_args		; buffer to copy string
	call 	get_argvstr
	jc 		args_error
	mov 	bx, ax
	mov 	byte[my_args + bx], 0
...
.another_string_var:
	...
	mov cx, di
	mov bx, si
	mov dx, 0			; the 2nd string is in the DS segment (BASIC)
	call os_string_join
...
.add_number_var:
	...
	mov ax, di
	mov cx, di
	mov dx, 1				; the 2nd string is in the FS segment (kernel)
	call os_string_join
...
.finish:
	...
	err_end_number		db "Error: unexpected zero-terminated in number or missing 'end'"
	...
	err_args		db "Error: Missing or invalid arguments",0
	err_files		db "Error: the file in the argument was not found!",0
	err_break		db "BREAK CALLED", 0

	line_num_starter	db " - line ", 0

	program_file 	db "program.bas",0
