...
.done:
	mov word [fs:.tmp_counter], cx	; Store count before restoring other registers
	popa

	mov ax, [fs:.tmp_counter]		; Put count back into AX before returning
...
.more:
	mov al, [ds:si]			; Transfer contents (at least one byte terminator)
	mov [es:di], al
	inc si
	inc di
...
os_string_to_int:
	...
	mov word [fs:.multiplier], 1	; Start with multiples of 1
...
.loop:
	...
	mul word [fs:.multiplier]		; Multiply by our multiplier

	add bx, ax			; Add it to BX

	push ax				; Multiply our multiplier by 10 for next char
	mov word ax, [fs:.multiplier]
	mov dx, 10
	mul dx
	mov word [fs:.multiplier], ax
	pop ax
...
.pop:
	...
	mov byte [fs:di], 0		; Zero-terminate string
...
.neg:
	...
	mov byte [fs:.t], '-'		; Add a minus sign to our string
...
os_get_date_string:
	...
	mov bx, [fs:fmt_date]		; BL = format code
