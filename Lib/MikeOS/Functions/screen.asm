; ==================================================================
; MikeOS -- The Mike Operating System kernel
; Copyright (C) 2006 - 2021 MikeOS Developers -- see doc/LICENSE.TXT
;
; SCREEN HANDLING SYSTEM CALLS
; ==================================================================

; ------------------------------------------------------------------
; os_print_string -- Displays text
; IN: SI = message location (zero-terminated string)
; OUT: Nothing (registers preserved)

os_print_string:
	pusha

	mov ah, 0Eh			; int 10h teletype function

.repeat:
	lodsb				; Get char from string
	cmp al, 0
	je .done			; If char is zero, end of string

	int 10h				; Otherwise, print it
	jmp .repeat			; And move on to next char

.done:
	popa
	jmp 	return.common


; ------------------------------------------------------------------
; os_clear_screen -- Clears the screen to background
; IN/OUT: Nothing (registers preserved)

os_clear_screen:
	pusha

	mov dx, 050Ch			; Position cursor at top-left
	intern 1
	call os_move_cursor

	mov ah, 6			; Scroll full-screen
	mov al, 0			; Normal white on black
	mov bh, 7			;
	mov cx, 050Ch			; Top-left
	mov dh, 22 		;24			; Bottom-right
	mov dl, 67 		;79
	int 10h
	
	mov byte[fs:SHELL.CursorRaw], 4
	mov byte[fs:SHELL.CursorCol], 12

	popa
	jmp 	return.common


; ------------------------------------------------------------------
; os_move_cursor -- Moves cursor in text mode
; IN: DH, DL = row, column; OUT: Nothing (registers preserved)

os_move_cursor:
	pusha

	mov bh, 0
	mov ah, 2
	int 10h				; BIOS interrupt to move cursor

	popa
	jmp 	return.common


; ------------------------------------------------------------------
; os_get_cursor_pos -- Return position of text cursor
; OUT: DH, DL = row, column

os_get_cursor_pos:
	pusha

	mov bh, 0
	mov ah, 3
	int 10h				; BIOS interrupt to get cursor position

	mov [fs:.tmp], dx
	popa
	mov dx, [fs:.tmp]
	jmp 	return.common


	.tmp dw 0


; ------------------------------------------------------------------
; os_print_horiz_line -- Draw a horizontal line on the screen
; IN: AX = line type (1 for double (-), otherwise single (=))
; OUT: Nothing (registers preserved)

os_print_horiz_line:
	pusha

	mov cx, ax			; Store line type param
	mov al, 196			; Default is single-line code

	cmp cx, 1			; Was double-line specified in AX?
	jne .ready
	mov al, 205			; If so, here's the code

.ready:
	mov cx, 0			; Counter
	mov ah, 0Eh			; BIOS output char routine

.restart:
	int 10h
	inc cx
	cmp cx, 80			; Drawn 80 chars yet?
	je .done
	jmp .restart

.done:
	popa
	jmp 	return.common


; ------------------------------------------------------------------
; os_show_cursor -- Turns on cursor in text mode
; IN/OUT: Nothing

os_show_cursor:
	pusha

	mov ch, 6
	mov cl, 7
	mov ah, 1
	mov al, 3
	int 10h

	popa
	jmp 	return.common


; ------------------------------------------------------------------
; os_hide_cursor -- Turns off cursor in text mode
; IN/OUT: Nothing

os_hide_cursor:
	pusha

	mov ch, 32
	mov ah, 1
	mov al, 3			; Must be video mode for buggy BIOSes!
	int 10h

	popa
	jmp 	return.common


; ------------------------------------------------------------------
; os_draw_block -- Render block of specified colour
; IN: BL/DL/DH/SI/DI = colour/start X pos/start Y pos/width/finish Y pos

os_draw_block:
	pusha
.more:
	intern 1
	call os_move_cursor		; Move to block starting position

	mov ah, 09h			; Draw colour section
	mov bh, 0
	mov cx, si
	mov al, ' '
	int 10h

	inc dh				; Get ready for next line

	mov ax, 0
	mov al, dh			; Get current Y position into DL
	cmp ax, di			; Reached finishing point (DI)?
	jne .more			; If not, keep drawing

	popa
	jmp 	return.common

; ------------------------------------------------------------------
; os_file_selector -- Show a file selection dialog
; IN: Nothing; OUT: AX = location of filename string (or carry set if Esc pressed)

os_file_selector:
	pusha

	mov word [fs:.filename], 0		; Terminate string in case user leaves without choosing

	intern 1
	mov ax, .buffer			; Get comma-separated list of filenames
	call os_get_file_list

	intern 1
	mov ax, .buffer			; Show those filenames in a list dialog box
	mov bx, .help_msg1
	mov cx, .help_msg2
	call os_list_dialog

	jc .esc_pressed

	dec ax				; Result from os_list_box starts from 1, but
					; for our file list offset we want to start from 0

	mov cx, ax
	mov bx, 0

	mov si, .buffer			; Get our filename from the list
.loop1:
	cmp bx, cx
	je .got_our_filename
	lodsb
	cmp al, ','
	je .comma_found
	jmp .loop1

.comma_found:
	inc bx
	jmp .loop1


.got_our_filename:			; Now copy the filename string
	mov di, .filename
.loop2:
	lodsb
	cmp al, ','
	je .finished_copying
	cmp al, 0
	je .finished_copying
	stosb
	jmp .loop2

.finished_copying:
	mov byte [di], 0		; Zero terminate the filename string

	popa

	mov ax, .filename

	jmp 	return.nocarry


.esc_pressed:				; Set carry flag if Escape was pressed
	popa
	jmp 	return.carry


	.buffer		times 1024 db 0

	.help_msg1	db 'Please select a file using the cursor', 0
	.help_msg2	db 'keys from the list below...', 0

	.filename	times 13 db 0


; ------------------------------------------------------------------
; os_list_dialog -- Show a dialog with a list of options
; IN: AX = comma-separated list of strings to show (zero-terminated),
;     BX = first help string, CX = second help string
; OUT: AX = number (starts from 1) of entry selected; carry set if Esc pressed

os_list_dialog:
	pusha

	push ax				; Store string list for now

	push cx				; And help strings
	push bx

	intern 1
	call os_hide_cursor


	mov cl, 0			; Count the number of entries in the list
	mov si, ax
.count_loop:
	lodsb
	cmp al, 0
	je .done_count
	cmp al, ','
	jne .count_loop
	inc cl
	jmp .count_loop

.done_count:
	inc cl
	mov byte [fs:.num_of_entries], cl

	intern 1
	mov bl, 00011111b		; White on red
	mov dl, 20			; Start X position
	mov dh, 5			; Start Y position
	mov si, 40			; Width
	mov di, 23			; Finish Y position
	call os_draw_block		; Draw option selector window

	intern 1
	mov dl, 21			; Show first line of help text...
	mov dh, 6
	call os_move_cursor

	intern 1
	pop si				; Get back first string
	call os_print_string

	intern 1
	inc dh				; ...and the second
	call os_move_cursor

	intern 1
	pop si
	call os_print_string


	pop si				; SI = location of option list string (pushed earlier)
	mov word [fs:.list_string], si


	; Now that we've drawn the list, highlight the currently selected
	; entry and let the user move up and down using the cursor keys

	mov byte [fs:.skip_num], 0		; Not skipping any lines at first showing

	mov dl, 25			; Set up starting position for selector
	mov dh, 7

	intern 1
	call os_move_cursor

.more_select:
	pusha
	intern 1
	mov bl, 01110001b		; Black on white for option list box
	mov dl, 21
	mov dh, 9
	mov si, 38
	mov di, 22
	call os_draw_block
	popa

	call .draw_black_bar

	mov word si, [fs:.list_string]
	call .draw_list

.another_key:
	intern 1
	call os_wait_for_key		; Move / select option
	cmp ah, 48h			; Up pressed?
	je .go_up
	cmp ah, 50h			; Down pressed?
	je .go_down
	cmp al, 13			; Enter pressed?
	je .option_selected
	cmp al, 27			; Esc pressed?
	je .esc_pressed
	jmp .more_select		; If not, wait for another key


.go_up:
	cmp dh, 7			; Already at top?
	jle .hit_top

	call .draw_white_bar

	intern 1
	mov dl, 25
	call os_move_cursor

	dec dh				; Row to select (increasing down)
	jmp .more_select


.go_down:				; Already at bottom of list?
	cmp dh, 20
	je .hit_bottom

	mov cx, 0
	mov byte cl, dh

	sub cl, 7
	inc cl
	add byte cl, [fs:.skip_num]

	mov byte al, [fs:.num_of_entries]
	cmp cl, al
	je .another_key

	call .draw_white_bar

	mov dl, 25
	call os_move_cursor

	inc dh
	jmp .more_select


.hit_top:
	mov byte cl, [fs:.skip_num]	; Any lines to scroll up?
	cmp cl, 0
	je .another_key			; If not, wait for another key

	dec byte [fs:.skip_num]		; If so, decrement lines to skip
	jmp .more_select


.hit_bottom:				; See if there's more to scroll
	mov cx, 0
	mov byte cl, dh

	sub cl, 7
	inc cl
	add byte cl, [fs:.skip_num]

	mov byte al, [fs:.num_of_entries]
	cmp cl, al
	je .another_key

	inc byte [fs:.skip_num]		; If so, increment lines to skip
	jmp .more_select



.option_selected:
	intern 1
	call os_show_cursor

	sub dh, 7

	mov ax, 0
	mov al, dh

	inc al				; Options start from 1
	add byte al, [fs:.skip_num]	; Add any lines skipped from scrolling

	mov word [fs:.tmp], ax		; Store option number before restoring all other regs

	popa

	intern 1
	call 	os_clear_screen
	mov word ax, [fs:.tmp]
	jmp 	return.nocarry	; Clear carry as Esc wasn't pressed

.esc_pressed:
	intern 1
	call os_show_cursor
	popa
	intern 1
	call 	os_clear_screen
	jmp 	return.carry	; Set carry for Esc



.draw_list:
	pusha

	intern 1
	mov dl, 23			; Get into position for option list text
	mov dh, 7
	call os_move_cursor


	mov cx, 0			; Skip lines scrolled off the top of the dialog
	mov byte cl, [fs:.skip_num]

.skip_loop:
	cmp cx, 0
	je .skip_loop_finished
.more_lodsb:
	lodsb
	cmp al, ','
	jne .more_lodsb
	dec cx
	jmp .skip_loop


.skip_loop_finished:
	mov bx, 0			; Counter for total number of options


.more:
	lodsb				; Get next character in file name, increment pointer

	cmp al, 0			; End of string?
	je .done_list

	cmp al, ','			; Next option? (String is comma-separated)
	je .newline

	mov ah, 0Eh
	int 10h
	jmp .more

.newline:
	intern 1
	mov dl, 23			; Go back to starting X position
	inc dh				; But jump down a line
	call os_move_cursor

	inc bx				; Update the number-of-options counter
	cmp bx, 14			; Limit to one screen of options
	jl .more

.done_list:
	popa
	intern 1
	call os_move_cursor

	ret



.draw_black_bar:
	pusha

	intern 1
	mov dl, 22
	call os_move_cursor

	mov ah, 09h			; Draw white bar at top
	mov bh, 0
	mov cx, 36
	mov bl, 00001111b		; White text on black background
	mov al, ' '
	int 10h

	popa
	ret



.draw_white_bar:
	pusha

	intern 1
	mov dl, 22
	call os_move_cursor

	mov ah, 09h			; Draw white bar at top
	mov bh, 0
	mov cx, 36
	mov bl, 01110001b		; Black text on white background
	mov al, ' '
	int 10h

	popa
	ret


	.tmp			dw 0
	.num_of_entries		db 0
	.skip_num		db 0
	.list_string		dw 0


; ------------------------------------------------------------------
; os_draw_background -- Clear screen with white top and bottom bars
; containing text, and a coloured middle section.
; IN: AX/BX = top/bottom string locations, CX = colour

os_draw_background:
	pusha

	push ax				; Store params to pop out later
	push bx
	push cx

	intern 1
	mov dl, 0
	mov dh, 0
	call os_move_cursor

	mov ah, 09h			; Draw white bar at top
	mov bh, 0
	mov cx, 80
	mov bl, 01110000b
	mov al, ' '
	int 10h

	intern 1
	mov dh, 1
	mov dl, 0
	call os_move_cursor

	mov ah, 09h			; Draw colour section
	mov cx, 1840
	pop bx				; Get colour param (originally in CX)
	mov bh, 0
	mov al, ' '
	int 10h

	intern 1
	mov dh, 24
	mov dl, 0
	call os_move_cursor

	mov ah, 09h			; Draw white bar at bottom
	mov bh, 0
	mov cx, 80
	mov bl, 01110000b
	mov al, ' '
	int 10h

	intern 1
	mov dh, 24
	mov dl, 1
	call os_move_cursor
	intern 1
	pop bx				; Get bottom string param
	mov si, bx
	call os_print_string

	intern 1
	mov dh, 0
	mov dl, 1
	call os_move_cursor
	intern 1
	pop ax				; Get top string param
	mov si, ax
	call os_print_string

	intern 1
	mov dh, 1			; Ready for app text
	mov dl, 0
	call os_move_cursor

	popa
	jmp 	return.common


; ------------------------------------------------------------------
; os_print_newline -- Reset cursor to start of next line
; IN: nothing

os_print_newline:
	pusha
	push 	ds

	push 	fs
	pop 	ds
	
	call 	SHELL.New_Line

	pop 	ds
	popa
	jmp 	return.common


; ------------------------------------------------------------------
; os_dump_registers -- Displays register contents in hex on the screen
; IN/OUT: AX/BX/CX/DX = registers to show

os_dump_registers:
	pusha

	intern 1
	call os_print_newline

    push bp
	push di
	push si
	push dx
	push cx
	push bx

	intern 1
	mov si, .ax_string
	call os_print_string
	intern 1
	call os_print_4hex

	intern 1
	pop ax
	mov si, .bx_string
	call os_print_string
	intern 1
	call os_print_4hex

	intern 1
	pop ax
	mov si, .cx_string
	call os_print_string
	intern 1
	call os_print_4hex

	intern 1
	pop ax
	mov si, .dx_string
	call os_print_string
	intern 1
	call os_print_4hex

	intern 1
	pop ax
	mov si, .si_string
	call os_print_string
	intern 1
	call os_print_4hex

	intern 1
	pop ax
	mov si, .di_string
	call os_print_string
	intern 1
	call os_print_4hex

	intern 1
    pop ax
    mov si, .bp_string
    call os_print_string
	intern 1
    call os_print_4hex

	intern 1
	call os_print_newline

	popa
	jmp 	return.common


	.ax_string		db 'AX:', 0
	.bx_string		db ' BX:', 0
	.cx_string		db ' CX:', 0
	.dx_string		db ' DX:', 0
	.si_string		db ' SI:', 0
	.di_string		db ' DI:', 0
    .bp_string              db ' BP:', 0


; ------------------------------------------------------------------
; os_input_dialog -- Get text string from user via a dialog box
; IN: AX = string location, BX = message to show; OUT: AX = string location

os_input_dialog:
	pusha

	push ax				; Save string location
	push bx				; Save message to show

	mov dh, 10			; First, draw red background box
	mov dl, 12

.redbox:				; Loop to draw all lines of box
	intern 1
	call os_move_cursor

	pusha
	mov ah, 09h
	mov bh, 0
	mov cx, 55
	mov bl, 01001111b		; White on red
	mov al, ' '
	int 10h
	popa

	inc dh
	cmp dh, 16
	je .boxdone
	jmp .redbox


.boxdone:
	intern 1
	mov dl, 14
	mov dh, 11
	call os_move_cursor

	intern 1
	pop bx				; Get message back and display it
	mov si, bx
	call os_print_string

	intern 1
	mov dl, 14
	mov dh, 13
	call os_move_cursor

	intern 1
	pop ax				; Get input string back
	mov bx, 50
	call os_input_string

	popa
	jmp 	return.common


; ------------------------------------------------------------------
; os_dialog_box -- Print dialog box in middle of screen, with button(s)
; IN: AX, BX, CX = string locations (set registers to 0 for no display)
; IN: DX = 0 for single 'OK' dialog, 1 for two-button 'OK' and 'Cancel'
; OUT: If two-button mode, AX = 0 for OK and 1 for cancel
; NOTE: Each string is limited to 40 characters

os_dialog_box:
	pusha

	mov [fs:.tmp], dx

	intern 1
	call os_hide_cursor

	mov dh, 9			; First, draw red background box
	mov dl, 19

.bluebox:				; Loop to draw all lines of box
	intern 1
	call os_move_cursor

	pusha
	mov ah, 09h
	mov bh, 0
	mov cx, 42
	mov bl, 00011111b		; White on Blue ..
	mov al, ' '
	int 10h
	popa

	inc dh
	cmp dh, 16
	je .boxdone
	jmp .bluebox


.boxdone:
	cmp ax, 0			; Skip string params if zero
	je .no_first_string
	intern 1
	mov dl, 20
	mov dh, 10
	call os_move_cursor

	intern 1
	mov si, ax			; First string
	call os_print_string

.no_first_string:
	cmp bx, 0
	je .no_second_string
	intern 1
	mov dl, 20
	mov dh, 11
	call os_move_cursor

	intern 1
	mov si, bx			; Second string
	call os_print_string

.no_second_string:
	cmp cx, 0
	je .no_third_string
	intern 1
	mov dl, 20
	mov dh, 12
	call os_move_cursor

	intern 1
	mov si, cx			; Third string
	call os_print_string

.no_third_string:
	mov dx, [fs:.tmp]
	cmp dx, 0
	je .one_button
	cmp dx, 1
	je .two_button


.one_button:
	intern 1
	mov bl, 01110000b		; Black on white
	mov dh, 14
	mov dl, 35
	mov si, 8
	mov di, 15
	call os_draw_block

	intern 1
	mov dl, 38			; OK button, centred at bottom of box
	mov dh, 14
	call os_move_cursor
	intern 1
	push ds
	push fs
	pop ds
	mov si, .ok_button_string
	call os_print_string
	pop ds

	jmp .one_button_wait


.two_button:
	intern 1
	mov bl, 11110000b		; Black on white
	mov dh, 14
	mov dl, 27
	mov si, 8
	mov di, 15
	call os_draw_block

	intern 1
	mov dl, 30			; OK button
	mov dh, 14
	call os_move_cursor
	intern 1
	push ds
	push fs
	pop ds
	mov si, .ok_button_string
	call os_print_string
	pop ds

	intern 1
	mov dl, 44			; Cancel button
	mov dh, 14
	call os_move_cursor
	intern 1
	push ds
	push fs
	pop ds
	mov si, .cancel_button_string
	call os_print_string
	pop ds

	mov cx, 0			; Default button = 0
	jmp .two_button_wait



.one_button_wait:
	intern 1
	call os_wait_for_key
	cmp al, 13			; Wait for enter key (13) to be pressed
	jne .one_button_wait

	intern 1
	call os_show_cursor

	popa
	intern 1
	call 	os_clear_screen
	jmp 	return.common


.two_button_wait:
	intern 1
	call os_wait_for_key

	cmp ah, 75			; Left cursor key pressed?
	jne .noleft

	intern 1
	mov bl, 11110000b		; Black on white
	mov dh, 14
	mov dl, 27
	mov si, 8
	mov di, 15
	call os_draw_block

	intern 1
	mov dl, 30			; OK button
	mov dh, 14
	call os_move_cursor
	intern 1
	push ds
	push fs
	pop ds
	mov si, .ok_button_string
	call os_print_string
	pop ds

	intern 1
	mov bl, 01001111b		; White on red for cancel button
	mov dh, 14
	mov dl, 42
	mov si, 9
	mov di, 15
	call os_draw_block

	intern 1
	mov dl, 44			; Cancel button
	mov dh, 14
	call os_move_cursor
	intern 1
	push ds
	push fs
	pop ds
	mov si, .cancel_button_string
	call os_print_string
	pop ds

	mov cx, 0			; And update result we'll return
	jmp .two_button_wait


.noleft:
	cmp ah, 77			; Right cursor key pressed?
	jne .noright

	intern 1
	mov bl, 01001111b		; Black on white
	mov dh, 14
	mov dl, 27
	mov si, 8
	mov di, 15
	call os_draw_block

	intern 1
	mov dl, 30			; OK button
	mov dh, 14
	call os_move_cursor
	intern 1
	push ds
	push fs
	pop ds
	mov si, .ok_button_string
	call os_print_string
	pop ds

	intern 1
	mov bl, 11110000b		; White on red for cancel button
	mov dh, 14
	mov dl, 43
	mov si, 8
	mov di, 15
	call os_draw_block

	intern 1
	mov dl, 44			; Cancel button
	mov dh, 14
	call os_move_cursor
	intern 1
	push ds
	push fs
	pop ds
	mov si, .cancel_button_string
	call os_print_string
	pop ds

	mov cx, 1			; And update result we'll return
	jmp .two_button_wait


.noright:
	cmp al, 13			; Wait for enter key (13) to be pressed
	jne .two_button_wait

	intern 1
	call os_show_cursor

	mov [fs:.tmp], cx			; Keep result after restoring all regs
	popa
	mov ax, [fs:.tmp]

	intern 1
	call 	os_clear_screen
	jmp 	return.common

	.ok_button_string	db 'OK', 0
	.cancel_button_string	db 'Cancel', 0
	.ok_button_noselect	db '   OK   ', 0
	.cancel_button_noselect	db '   Cancel   ', 0

	.tmp dw 0


; ------------------------------------------------------------------
; os_print_space -- Print a space to the screen
; IN/OUT: Nothing

os_print_space:
	pusha

	mov ah, 0Eh			; BIOS teletype function
	mov al, 20h			; Space is character 20h
	int 10h

	popa
	jmp 	return.common


; ------------------------------------------------------------------
; os_dump_string -- Dump string as hex bytes and printable characters
; IN: SI = points to string to dump

os_dump_string:
	pusha

	mov bx, si			; Save for final print

.line:
	mov di, si			; Save current pointer
	mov cx, 0			; Byte counter

.more_hex:
	lodsb
	cmp al, 0
	je .chr_print

	intern 1
	call os_print_2hex
	intern 1
	call os_print_space		; Single space most bytes
	inc cx

	cmp cx, 8
	jne .q_next_line

	intern 1
	call os_print_space		; Double space centre of line
	jmp .more_hex

.q_next_line:
	cmp cx, 16
	jne .more_hex

.chr_print:
	cmp cx, 16			; full line?
	jae .chr_cont
	mov ax, 16			; no, partial line, pad hex display with spaces
	sub ax, cx
	mov cx, ax
	mov dx, ax
	shl ax, 1
	add cx, ax
	cmp dx, 8			; less than 1/2 line?
	jbe .chr_c1
	inc cx				; yes, add space
.chr_c1:
	intern 1
	call os_print_space
	loop .chr_c1
.chr_cont:
	intern 1
	call os_print_space
	mov ah, 0Eh			; BIOS teletype function
	mov al, '|'                     ; Break between hex and character
	int 10h
	intern 1
	call os_print_space

	mov si, di			; Go back to beginning of this line
	mov cx, 0

.more_chr:
	lodsb
	cmp al, 0
	je .done

	cmp al, ' '
	jae .tst_high

	jmp short .not_printable

.tst_high:
	cmp al, '~'
	jbe .output

.not_printable:
	mov al, '.'

.output:
	mov ah, 0Eh
	int 10h

	inc cx
	cmp cx, 16
	jl .more_chr

	intern 1
	call os_print_newline		; Go to next line
	jmp .line

.done:
	intern 1
	call os_print_newline		; Go to next line

	popa
	jmp 	return.common


; ------------------------------------------------------------------
; os_print_digit -- Displays contents of AX as a single digit
; Works up to base 37, ie digits 0-Z
; IN: AX = "digit" to format and print

os_print_digit:
	pusha

	cmp ax, 9                       ; There is a break in ASCII table between 9 and A
	jle .digit_format

	add ax, 'A'-'9'-1		; Correct for the skipped punctuation

.digit_format:
	add ax, '0'			; 0 will display as '0', etc.	

	mov ah, 0Eh			; May modify other registers
	int 10h

	popa
	jmp 	return.common


; ------------------------------------------------------------------
; os_print_1hex -- Displays low nibble of AL in hex format
; IN: AL = number to format and print

os_print_1hex:
	pusha

	intern 1
	and ax, 0Fh			; Mask off data to display
	call os_print_digit

	popa
	jmp 	return.common


; ------------------------------------------------------------------
; os_print_2hex -- Displays AL in hex format
; IN: AL = number to format and print

os_print_2hex:
	pusha

	intern 1
	push ax				; Output high nibble
	shr ax, 4
	call os_print_1hex

	intern 1
	pop ax				; Output low nibble
	call os_print_1hex

	popa
	jmp 	return.common


; ------------------------------------------------------------------
; os_print_4hex -- Displays AX in hex format
; IN: AX = number to format and print

os_print_4hex:
	pusha

	intern 1
	push ax				; Output high byte
	mov al, ah
	call os_print_2hex

	intern 1
	pop ax				; Output low byte
	call os_print_2hex

	popa
	jmp 	return.common


; ------------------------------------------------------------------
; os_input_string --- Get a string from keyboard input
; IN: AX = output address, BX = maximum bytes of output string
; OUT: nothing

os_input_string:
	pusha

	; If the character count is zero, don't do anything.
	cmp bx, 0
	je .done

	mov di, ax			; DI = Current position in buffer
	
	dec bx				; BX = Maximum characters in string
	mov cx, bx			; CX = Remaining character count

.get_char:
	intern 1
	call os_wait_for_key

	cmp al, 8
	je .backspace

	cmp al, 13			; The ENTER key ends the prompt
	je .end_string

	; Do not add any characters if the maximum size has been reached.
	jcxz .get_char

	; Only add printable characters (ASCII Values 32-126)
	cmp al, ' '
	jb .get_char

	cmp al, 126
	ja .get_char

	call .add_char

	dec cx
	jmp .get_char

.end_string:
	mov al, 0
	stosb

.done:
	popa
	jmp 	return.common

.backspace:
	; Check if there are any characters to backspace
	cmp cx, bx 
	jae .get_char

	inc cx				; Increase characters remaining

	call .reverse_cursor		; Move back to the previous character
	mov al, ' '			; Print a space on the character
	call .add_char
	call .reverse_cursor		; Now move the cursor back again

	jmp .get_char

.reverse_cursor:
	dec di				; Move the output pointer backwards
	
	intern 1
	call os_get_cursor_pos
	cmp dl, 0			; Is the cursor at the start of line?
	je .back_line

	intern 1
	dec dl				; If not, just decrease the column
	call os_move_cursor
	ret

.back_line:
	intern 1
	dec dh				; Otherwise, move the cursor to the end
	mov dl, 79			; of the previous line.
	call os_move_cursor
	ret


.add_char:
	stosb
	mov ah, 0x0E			; Teletype Function
	mov bh, 0			; Video Page 0
	push bp				; Some BIOS's may mess up BP
	int 0x10
	pop bp
	ret


	


; ==================================================================

