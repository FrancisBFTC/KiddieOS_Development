...
os_seed_random:
	...
	mov word [fs:os_random_seed], bx	; Seed will be something like 0x4435 (if it
...
.generate_random:
	...
	mov ax, [fs:os_random_seed]
	mov dx, 23167
	mul dx				; DX:AX = AX * DX
	add ax, 12409
	adc dx, 0
	mov [fs:os_random_seed], ax
...
os_bcd_to_int:
	...
	popa
	mov ax, [fs:.tmp]			; And return it in AX!
	jmp 	return.common
