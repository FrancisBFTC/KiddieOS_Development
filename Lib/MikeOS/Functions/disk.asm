...
os_get_file_list:
	...
	jg .cont1			; read directory error will ireturn
	mov bx, 1			; ireturn an 'empty' array
  .cont1:
	mov cx, bx
  ...
  .fine:
	...
	pop es
	pop ds
	iclc				; directory read o.k., at least one byte in buffer
	iret
...
;	mov ax, 499			; Root dir starts at logical sector 19
...
;	istc				; ireturn with size = 0 and carry set
;	iret


;.found_file_to_load:			; Now fetch cluster and load FAT into RAM
;	mov eax, [di+28]		; Store file size to ireturn to calling routine
...
;	mov ax, 7			; Sector 1 = first sector of first FAT
;	call disk_convert_l2hts
...
;
;	call disk_reset_floppy		; Otherwise, reset floppy and iretry
...
os_write_file:
	...
	call os_file_exists		; Don't overwrite a file if it exists!
					...
					; ireturns not found (CY) if error or not found
...
os_remove_file:
	...
	call disk_get_root_entry	; Entry will be ireturned in DI
...
.okay:
	...
	; get the file size going to end of file
	mov 	ah, 42h
	mov 	al, 2
	mov 	cx, 0
	mov 	dx, 0
	...
	mov 	ebx, [fs:.tmp]		; return the size
	jmp 	return.nocarry
...
.to_next_root_entry:
	...
	istc				; Set carry if entry not found
	iret
...
.read_failure:
	popa
	istc				; Set carry flag (for failure)
	iret
...
; ******************************************************************
	...
	error_debug db "Error: Erro de Leitura DISK.asm inclusion",0
